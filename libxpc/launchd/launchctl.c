...
{	
	...
	if (plistReadStream) {
		...
		plist = CFPropertyListCreateWithStream(NULL, plistReadStream, 0, kCFPropertyListImmutable, &plistFormat, &errString);
		if (!plist) {
			launchctl_log_CFString(LOG_ERR, errString);
			CFRelease(errString);
		}
...
WriteMyPropertyListToFile(CFPropertyListRef plist, const char *posixfile)
{
	...
	resourceData = CFPropertyListCreateWithData(<#CFAllocatorRef allocator#>, <#CFDataRef data#>, <#CFOptionFlags options#>, <#CFPropertyListFormat *format#>, <#CFErrorRef *error#>);
